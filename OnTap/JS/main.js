...
function checkMaBenhNhan() {
    ...
    if (regex.test(maBenhNhan)) {
        ...
        $(".ma-mes").html("Mã bệnh nhân phải có dạng: BN-YYYYY (BN cố định, YYYYY là 5 số bất kì)");
        return false;
    }
}
...
function checkMatKhau() {
    ...
    if (regex.test(matKhau)) {
        ...
        $(".mk-mes").html("Mật khẩu phải từ 6 kí tự trở lên (Bất kỳ trừ kí tự đặc biệt)");
        return false;
    }
}
...
function themMoi() {
    ...
    stt++;
    const addNew = "<tr><td>" + stt + "</td><td>" +
        ma + "</td><td>" + matKhau + "</td><td>" +
        ngayKham + "</td><td>" + phuThu + "</td><td>" +
        chuyenKhoa + "</td></tr>"
